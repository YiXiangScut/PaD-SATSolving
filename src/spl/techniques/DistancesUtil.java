...
public class DistancesUtil {
    ...
    public static double getDiffDistance(Product p1, Product p2) {
        return Math.abs(p1.getSelectedNumber() - p2.getSelectedNumber())/p1.size();
    }
  ...
  //Used in sampling products
    public static double getWeightedDistance (Product p1, Product p2) {
        ...
        if (p1.getSelectedNumber() +  p2.getSelectedNumber() > SPL_sampler.getInstance().numFeatures) {
        	delta = 1.0 / ( SPL_sampler.getInstance().numFeatures - Math.min(p1.getSelectedNumber(), p2.getSelectedNumber()));
        }
        		
        dist = 0.5 *  (1.0 - intersectionSize / p1.size() ) * delta // getHammingDistance
        	 + 0.5 * ((double)(Math.abs(p1.getSelectedNumber() - p2.getSelectedNumber())) /(SPL_sampler.getInstance().numFeatures)) ;
        return dist;
...
//        return p1.size() - intersectionSize;
